...
proc means data=&INFILE. nmiss mean median std;
var _numeric_ ;
run;
...
proc print data=&TEMPFILE.(obs=25);
var FixedAcidity;
run;
...
proc contents data=&FIXFILE.;
run;

proc print data=&FIXFILE.(obs=25);
run;

proc means data=&FIXFILE. nmiss mean median std;
var _numeric_ ;
run;
...
proc print data=&FIXFILE.(obs=50);
...
proc print data=SCOREFILE(obs=25);
var TARGET X_GENMOD_NB P_GENMOD_NB P_REGRESSION X_LOGIT_PROB X_GENMOD_HURDLE P_LOGIT_PROB P_GENMOD_HURDLE P_HURDLE P_ENSEMBLE ;
run;
